...
{
    private static final SimpleCommandExceptionType OBJECTIVE_ALREADY_EXISTS_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.scoreboard.objectives.add.duplicate"));
    private static final SimpleCommandExceptionType DISPLAY_ALREADY_CLEAR_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.scoreboard.objectives.display.alreadyEmpty"));
    private static final SimpleCommandExceptionType DISPLAY_ALREADY_SET_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.scoreboard.objectives.display.alreadySet"));
    private static final SimpleCommandExceptionType ENABLE_TRIGGER_FAILED = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.scoreboard.players.enable.failed"));
    private static final SimpleCommandExceptionType ENABLE_TRIGGER_INVALID = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.scoreboard.players.enable.invalid"));
    ...
    {
        return new TranslationTextComponent("cmd.scoreboard.players.get.null", p_208907_0_, p_208907_1_);
    });
    ...
    private static int getPlayerScore(CommandSource source, String player, ScoreObjective objective) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.get.success", player, score.getScorePoints(), objective.func_197890_e()), false);
    ...
    private static int applyScoreOperation(CommandSource source, Collection<String> targetEntities, ScoreObjective targetObjectives, OperationArgument.IOperation operation, Collection<String> sourceEntities, ScoreObjective sourceObjective) throws CommandSyntaxException
    {
        ...
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.operation.success.single", targetObjectives.func_197890_e(), targetEntities.iterator().next(), i), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.operation.success.multiple", targetObjectives.func_197890_e(), targetEntities.size()), true);
        }
    ...
    private static int enableTrigger(CommandSource source, Collection<String> targets, ScoreObjective objective) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            else
            {
                if (targets.size() == 1)
                {
                    source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.enable.success.single", objective.func_197890_e(), targets.iterator().next()), true);
                }
                else
                {
                    source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.enable.success.multiple", objective.func_197890_e(), targets.size()), true);
                }
    ...
    private static int resetPlayerAllScores(CommandSource source, Collection<String> targets)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.reset.all.single", targets.iterator().next()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.reset.all.multiple", targets.size()), true);
        }
    ...
    private static int resetPlayerScore(CommandSource source, Collection<String> targets, ScoreObjective objective)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.reset.specific.single", objective.func_197890_e(), targets.iterator().next()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.reset.specific.multiple", objective.func_197890_e(), targets.size()), true);
        }
    ...
    private static int setPlayerScore(CommandSource source, Collection<String> targets, ScoreObjective objective, int newValue)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.set.success.single", objective.func_197890_e(), targets.iterator().next(), newValue), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.set.success.multiple", objective.func_197890_e(), targets.size(), newValue), true);
        }
    ...
    private static int addToPlayerScore(CommandSource source, Collection<String> targets, ScoreObjective objective, int amount)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.add.success.single", amount, objective.func_197890_e(), targets.iterator().next(), i), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.add.success.multiple", amount, objective.func_197890_e(), targets.size()), true);
        }
    ...
    private static int removeFromPlayerScore(CommandSource source, Collection<String> targets, ScoreObjective objective, int amount)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.remove.success.single", amount, objective.func_197890_e(), targets.iterator().next(), i), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.remove.success.multiple", amount, objective.func_197890_e(), targets.size()), true);
        }
    ...
    private static int listPlayers(CommandSource source)
    {
        ...
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.list.success", collection.size(), TextComponentUtils.makeGreenSortedList(collection)), false);
        }
    ...
    private static int listPlayerScores(CommandSource source, String player)
    {
        ...
        if (map.isEmpty())
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.list.entity.empty", player), false);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.list.entity.success", player, map.size()), false);

            for (Entry<ScoreObjective, Score> entry : map.entrySet())
            {
                source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.players.list.entity.entry", entry.getKey().func_197890_e(), entry.getValue().getScorePoints()), false);
            }
        }
    ...
    private static int clearObjectiveDisplaySlot(CommandSource source, int slotId) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.objectives.display.cleared", Scoreboard.getDisplaySlotStrings()[slotId]), true);
            return 0;
        }
    }
    ...
    private static int setObjectiveDisplaySlot(CommandSource source, int slotId, ScoreObjective objective) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.objectives.display.set", Scoreboard.getDisplaySlotStrings()[slotId], objective.getDisplayName()), true);
            return 0;
        }
    }
    ...
    private static int setDisplayName(CommandSource source, ScoreObjective objective, ITextComponent displayName)
    {
        if (!objective.getDisplayName().equals(displayName))
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.objectives.modify.displayname", objective.getName(), objective.func_197890_e()), true);
        }
    ...
    private static int setRenderType(CommandSource source, ScoreObjective objective, ScoreCriteria.RenderType renderType)
    {
        if (objective.getRenderType() != renderType)
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.objectives.modify.rendertype", objective.func_197890_e()), true);
        }
    ...
    private static int listObjectives(CommandSource source)
    {
        ...
        {
            source.sendFeedback(new TranslationTextComponent("cmd.scoreboard.objectives.list.success", collection.size(), TextComponentUtils.func_240649_b_(collection, ScoreObjective::func_197890_e)), false);
        }
