...
{
    private static final SimpleCommandExceptionType SET_POINTS_INVALID_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.experience.set.points.invalid"));
    ...
    private static int queryExperience(CommandSource source, ServerPlayerEntity player, ExperienceCommand.Type type)
    {
        ...
        source.sendFeedback(new TranslationTextComponent("cmd.experience.query." + type.name, player.getDisplayName(), i), false);
        return i;
    }
    ...
    private static int addExperience(CommandSource source, Collection <? extends ServerPlayerEntity > targets, int amount, ExperienceCommand.Type type)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.experience.add." + type.name + ".success.single", amount, targets.iterator().next().getDisplayName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.experience.add." + type.name + ".success.multiple", amount, targets.size()), true);
        }
    ...
    private static int setExperience(CommandSource source, Collection <? extends ServerPlayerEntity > targets, int amount, ExperienceCommand.Type type) throws CommandSyntaxException
    {
        ...
        else
        {
            if (targets.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.experience.set." + type.name + ".success.single", amount, targets.iterator().next().getDisplayName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.experience.set." + type.name + ".success.multiple", amount, targets.size()), true);
            }
