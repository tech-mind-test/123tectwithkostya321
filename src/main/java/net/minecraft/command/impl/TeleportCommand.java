...
{
    private static final SimpleCommandExceptionType field_241077_a_ = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.teleport.invalidPosition"));
    ...
    private static int teleportToEntity(CommandSource source, Collection <? extends Entity > targets, Entity destination) throws CommandSyntaxException
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.teleport.success.entity.single", targets.iterator().next().getDisplayName(), destination.getDisplayName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.teleport.success.entity.multiple", targets.size(), destination.getDisplayName()), true);
        }
    ...
    private static int teleportToPos(CommandSource source, Collection <? extends Entity > targets, ServerWorld worldIn, ILocationArgument position, @Nullable ILocationArgument rotationIn, @Nullable TeleportCommand.Facing facing) throws CommandSyntaxException
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.teleport.success.location.single", targets.iterator().next().getDisplayName(), vector3d.x, vector3d.y, vector3d.z), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.teleport.success.location.multiple", targets.size(), vector3d.x, vector3d.y, vector3d.z), true);
        }
