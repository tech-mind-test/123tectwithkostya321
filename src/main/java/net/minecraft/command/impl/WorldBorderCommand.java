...
{
    private static final SimpleCommandExceptionType CENTER_NO_CHANGE = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.worldborder.center.failed"));
    private static final SimpleCommandExceptionType SIZE_NO_CHANGE = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.worldborder.set.failed.nochange"));
    private static final SimpleCommandExceptionType SIZE_TOO_SMALL = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.worldborder.set.failed.small."));
    private static final SimpleCommandExceptionType SIZE_TOO_BIG = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.worldborder.set.failed.big."));
    private static final SimpleCommandExceptionType WARNING_TIME_NO_CHANGE = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.worldborder.warning.time.failed"));
    private static final SimpleCommandExceptionType WARNING_DISTANCE_NO_CHANGE = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.worldborder.warning.distance.failed"));
    private static final SimpleCommandExceptionType DAMAGE_BUFFER_NO_CHANGE = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.worldborder.damage.buffer.failed"));
    private static final SimpleCommandExceptionType DAMAGE_AMOUNT_NO_CHANGE = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.worldborder.damage.amount.failed"));
    ...
    private static int getSize(CommandSource source)
    {
        ...
        source.sendFeedback(new TranslationTextComponent("cmd.worldborder.get", String.format(Locale.ROOT, "%.0f", d0)), false);
    ...
    private static int setSize(CommandSource source, double newSize, long time) throws CommandSyntaxException
    {
        ...
        else
        {
            if (time > 0L)
            {
                ...
                if (newSize > d0)
                {
                    source.sendFeedback(new TranslationTextComponent("cmd.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", newSize), Long.toString(time / 1000L)), true);
                }
                else
                {
                    source.sendFeedback(new TranslationTextComponent("cmd.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", newSize), Long.toString(time / 1000L)), true);
                }
            }
            else
            {
                worldborder.setTransition(newSize);
                source.sendFeedback(new TranslationTextComponent("cmd.worldborder.set.immediate", String.format(Locale.ROOT, "%.1f", newSize)), true);
            }
