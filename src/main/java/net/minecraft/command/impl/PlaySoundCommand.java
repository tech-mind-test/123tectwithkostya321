...
{
    private static final SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.playsound.failed"));
        ...
        while (true)
        {
            ...
            while (true)
            {
                if (!iterator.hasNext())
                {
                    ...
                    if (targets.size() == 1)
                    {
                        source.sendFeedback(new TranslationTextComponent("cmd.playsound.success.single", soundIn, targets.iterator().next().getDisplayName()), true);
                    }
                    else
                    {
                        source.sendFeedback(new TranslationTextComponent("cmd.playsound.success.multiple", soundIn, targets.size()), true);
                    }
