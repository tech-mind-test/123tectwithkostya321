...
public class BanListCommand
{
    ...
    private static int sendBanList(CommandSource source, Collection <? extends BanEntry<? >> bannedPlayerList)
    {
        ...
        {
            source.sendFeedback(new TranslationTextComponent("cmd.banlist.none"), false);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.banlist.list", bannedPlayerList.size()), false);

            for (BanEntry<?> banentry : bannedPlayerList)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.banlist.entry", banentry.getDisplayName(), banentry.getBannedBy(), banentry.getBanReason()), false);
            }
        }
