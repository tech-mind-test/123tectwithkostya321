...
public class BanIpCommand
{
    ...
    private static final SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.banip.failed"));
    ...
    private static int banIpAddress(CommandSource source, String ip, @Nullable ITextComponent reason) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            if (!list.isEmpty())
            {
                source.sendFeedback(new TranslationTextComponent("cmd.banip.info", list.size(), EntitySelector.joinNames(list)), true);
            }
