    ...
    private static int clearInventory(CommandSource source, Collection<ServerPlayerEntity> targetPlayers, Predicate<ItemStack> itemPredicateIn, int maxCount) throws CommandSyntaxException
    {
        ...
        else
        {
            if (maxCount == 0)
            {
                if (targetPlayers.size() == 1)
                {
                    source.sendFeedback(new TranslationTextComponent("cmd.clear.test.single", i, targetPlayers.iterator().next().getDisplayName()), true);
                }
                else
                {
                    source.sendFeedback(new TranslationTextComponent("cmd.clear.test.multiple", i, targetPlayers.size()), true);
                }
            }
            else if (targetPlayers.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.clear.success.single", i, targetPlayers.iterator().next().getDisplayName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.clear.success.multiple", i, targetPlayers.size()), true);
            }
