...
{
    private static final SimpleCommandExceptionType ADD_FAILED = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.tag.add.failed"));
    private static final SimpleCommandExceptionType REMOVE_FAILED = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.tag.remove.failed"));
    ...
    private static int addTag(CommandSource source, Collection <? extends Entity > entities, String tagName) throws CommandSyntaxException
    {
        ...
        else
        {
            if (entities.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.tag.add.success.single", tagName, entities.iterator().next().getDisplayName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.tag.add.success.multiple", tagName, entities.size()), true);
            }
    ...
    private static int removeTag(CommandSource source, Collection <? extends Entity > entities, String tagName) throws CommandSyntaxException
    {
        ...
        else
        {
            if (entities.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.tag.remove.success.single", tagName, entities.iterator().next().getDisplayName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.tag.remove.success.multiple", tagName, entities.size()), true);
            }
    ...
    private static int listTags(CommandSource source, Collection <? extends Entity > entities)
    {
        ...
        if (entities.size() == 1)
        {
            ...
            if (set.isEmpty())
            {
                source.sendFeedback(new TranslationTextComponent("cmd.tag.list.single.empty", entity1.getDisplayName()), false);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.tag.list.single.success", entity1.getDisplayName(), set.size(), TextComponentUtils.makeGreenSortedList(set)), false);
            }
        }
        else if (set.isEmpty())
        {
            source.sendFeedback(new TranslationTextComponent("cmd.tag.list.multiple.empty", entities.size()), false);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.tag.list.multiple.success", entities.size(), set.size(), TextComponentUtils.makeGreenSortedList(set)), false);
        }
