...
{
    private static final SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.pardon.failed"));
    ...
    private static int unbanPlayers(CommandSource source, Collection<GameProfile> gameProfiles) throws CommandSyntaxException
    {
        ...
        for (GameProfile gameprofile : gameProfiles)
        {
            if (banlist.isBanned(gameprofile))
            {
                ...
                ++i;
                source.sendFeedback(new TranslationTextComponent("cmd.pardon.success", TextComponentUtils.getDisplayName(gameprofile)), true);
            }
        }
