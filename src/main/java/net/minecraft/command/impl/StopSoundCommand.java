...
public class StopSoundCommand
{
    ...
    private static int stopSound(CommandSource source, Collection<ServerPlayerEntity> targets, @Nullable SoundCategory category, @Nullable ResourceLocation soundIn)
    {
        ...
        if (category != null)
        {
            if (soundIn != null)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.stopsound.success.source.sound", soundIn, category.getName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.stopsound.success.source.any", category.getName()), true);
            }
        }
        else if (soundIn != null)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.stopsound.success.sourceless.sound", soundIn), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.stopsound.success.sourceless.any"), true);
        }
