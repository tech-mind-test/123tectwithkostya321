...
public class FunctionCommand
{
    ...
    private static int executeFunctions(CommandSource source, Collection<FunctionObject> functions)
    {
        ...
        if (functions.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.function.success.single", i, functions.iterator().next().getId()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.function.success.multiple", i, functions.size()), true);
        }
