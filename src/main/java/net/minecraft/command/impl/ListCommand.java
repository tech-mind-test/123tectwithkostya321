...
public class ListCommand
{
    ...
    private static int listUUIDs(CommandSource source)
    {
        ...
        {
            return new TranslationTextComponent("cmd.list.nameAndId", p_244373_0_.getName(), p_244373_0_.getGameProfile().getId());
        });
    }
    ...
    private static int listPlayers(CommandSource source, Function<ServerPlayerEntity, ITextComponent> nameExtractor)
    {
        ...
        source.sendFeedback(new TranslationTextComponent("cmd.list.players", list.size(), playerlist.getMaxPlayers(), itextcomponent), false);
        return list.size();
    }
}
