...
{
    public static final SimpleCommandExceptionType BLOCK_FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.replaceitem.block.failed"));
    public static final DynamicCommandExceptionType INAPPLICABLE_SLOT_EXCEPTION = new DynamicCommandExceptionType((p_211409_0_) ->
    {
        return new TranslationTextComponent("cmd.replaceitem.slot.inapplicable", p_211409_0_);
    });
    ...
    {
        return new TranslationTextComponent("cmd.replaceitem.entity.failed", p_211411_0_, p_211411_1_);
    });
    ...
    private static int replaceItemBlock(CommandSource source, BlockPos pos, int slotIn, ItemStack newStack) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            if (slotIn >= 0 && slotIn < iinventory.getSizeInventory())
            {
                ...
                source.sendFeedback(new TranslationTextComponent("cmd.replaceitem.block.success", pos.getX(), pos.getY(), pos.getZ(), newStack.getTextComponent()), true);
                return 1;
            }
            else
            {
    ...
    private static int replaceItemEntities(CommandSource source, Collection <? extends Entity > targets, int slotIn, ItemStack newStack) throws CommandSyntaxException
    {
        ...
        else
        {
            if (list.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.replaceitem.entity.success.single", list.iterator().next().getDisplayName(), newStack.getTextComponent()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.replaceitem.entity.success.multiple", list.size(), newStack.getTextComponent()), true);
            }
