...
public class BossBarCommand
{
    ...
    {
        return new TranslationTextComponent("cmd.bossbar.create.failed", p_208783_0_);
    });
    private static final DynamicCommandExceptionType NO_BOSSBAR_WITH_ID = new DynamicCommandExceptionType((p_208782_0_) ->
    {
        return new TranslationTextComponent("cmd.bossbar.unknown", p_208782_0_);
    });
    private static final SimpleCommandExceptionType PLAYERS_ALREADY_ON_BOSSBAR = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.bossbar.set.players.unchanged"));
    private static final SimpleCommandExceptionType ALREADY_NAME_OF_BOSSBAR = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.bossbar.set.name.unchanged"));
    private static final SimpleCommandExceptionType ALREADY_COLOR_OF_BOSSBAR = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.bossbar.set.color.unchanged"));
    private static final SimpleCommandExceptionType ALREADY_STYLE_OF_BOSSBAR = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.bossbar.set.style.unchanged"));
    private static final SimpleCommandExceptionType ALREADY_VALUE_OF_BOSSBAR = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.bossbar.set.value.unchanged"));
    private static final SimpleCommandExceptionType ALREADY_MAX_OF_BOSSBAR = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.bossbar.set.max.unchanged"));
    private static final SimpleCommandExceptionType BOSSBAR_ALREADY_HIDDEN = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.bossbar.set.visibility.unchanged.hidden"));
    private static final SimpleCommandExceptionType BOSSBAR_ALREADY_VISIBLE = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.bossbar.set.visibility.unchanged.visible"));
    ...
    {
        source.sendFeedback(new TranslationTextComponent("cmd.bossbar.get.value", bossbar.getFormattedName(), bossbar.getValue()), true);
        return bossbar.getValue();
    }

    private static int getMax(CommandSource source, CustomServerBossInfo bossbar)
    {
        source.sendFeedback(new TranslationTextComponent("cmd.bossbar.get.max", bossbar.getFormattedName(), bossbar.getMax()), true);
    ...
    private static int getVisibility(CommandSource source, CustomServerBossInfo bossbar)
    {
        ...
        {
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.get.visible.visible", bossbar.getFormattedName()), true);
            return 1;
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.get.visible.hidden", bossbar.getFormattedName()), true);
            return 0;
        }
    }
    ...
    private static int getPlayers(CommandSource source, CustomServerBossInfo bossbar)
    {
        ...
        {
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.get.players.none", bossbar.getFormattedName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.get.players.some", bossbar.getFormattedName(), bossbar.getPlayers().size(), TextComponentUtils.func_240649_b_(bossbar.getPlayers(), PlayerEntity::getDisplayName)), true);
        }
    ...
    private static int setVisibility(CommandSource source, CustomServerBossInfo bossbar, boolean visible) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            if (visible)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.visible.success.visible", bossbar.getFormattedName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.visible.success.hidden", bossbar.getFormattedName()), true);
            }
    ...
    private static int setValue(CommandSource source, CustomServerBossInfo bossbar, int value) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.value.success", bossbar.getFormattedName(), value), true);
            return value;
        }
    }
    ...
    private static int setMax(CommandSource source, CustomServerBossInfo bossbar, int max) throws CommandSyntaxException
    {
        ...
        {
            bossbar.setMax(max);
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.max.success", bossbar.getFormattedName(), max), true);
            return max;
        }
    }
    ...
    private static int setColor(CommandSource source, CustomServerBossInfo bossbar, BossInfo.Color color) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.color.success", bossbar.getFormattedName()), true);
            return 0;
        }
    }
    ...
    private static int setStyle(CommandSource source, CustomServerBossInfo bossbar, BossInfo.Overlay styleIn) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.style.success", bossbar.getFormattedName()), true);
            return 0;
        }
    }
    ...
    private static int setName(CommandSource source, CustomServerBossInfo bossbar, ITextComponent name) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.name.success", bossbar.getFormattedName()), true);
            return 0;
        }
    }
    ...
    private static int setPlayers(CommandSource source, CustomServerBossInfo bossbar, Collection<ServerPlayerEntity> players) throws CommandSyntaxException
    {
        ...
        else
        {
            if (bossbar.getPlayers().isEmpty())
            {
                source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.players.success.none", bossbar.getFormattedName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.bossbar.set.players.success.some", bossbar.getFormattedName(), players.size(), TextComponentUtils.func_240649_b_(players, PlayerEntity::getDisplayName)), true);
            }
    ...
    private static int listBars(CommandSource source)
    {
        ...
        {
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.list.bars.none"), false);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.list.bars.some", collection.size(), TextComponentUtils.func_240649_b_(collection, CustomServerBossInfo::getFormattedName)), false);
        }
    ...
    private static int createBossbar(CommandSource source, ResourceLocation id, ITextComponent displayName) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            source.sendFeedback(new TranslationTextComponent("cmd.bossbar.create.success", customserverbossinfo.getFormattedName()), true);
    ...
    private static int removeBossbar(CommandSource source, CustomServerBossInfo bossbar)
    {
        ...
        source.sendFeedback(new TranslationTextComponent("cmd.bossbar.remove.success", bossbar.getFormattedName()), true);
        return customserverbossinfomanager.getBossbars().size();
    }
