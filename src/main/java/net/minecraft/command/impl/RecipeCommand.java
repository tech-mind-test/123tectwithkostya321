...
{
    private static final SimpleCommandExceptionType GIVE_FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.recipe.give.failed"));
    private static final SimpleCommandExceptionType TAKE_FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.recipe.take.failed"));
    ...
    private static int giveRecipes(CommandSource source, Collection<ServerPlayerEntity> targets, Collection < IRecipe<? >> recipes) throws CommandSyntaxException
    {
        ...
        else
        {
            if (targets.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.recipe.give.success.single", recipes.size(), targets.iterator().next().getDisplayName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.recipe.give.success.multiple", recipes.size(), targets.size()), true);
            }
    ...
    private static int takeRecipes(CommandSource source, Collection<ServerPlayerEntity> targets, Collection < IRecipe<? >> recipes) throws CommandSyntaxException
    {
        ...
        else
        {
            if (targets.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.recipe.take.success.single", recipes.size(), targets.iterator().next().getDisplayName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.recipe.take.success.multiple", recipes.size(), targets.size()), true);
            }
