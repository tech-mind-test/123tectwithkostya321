...
public class KillCommand
{
    ...
    private static int killEntities(CommandSource source, Collection <? extends Entity > targets)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.kill.success.single", targets.iterator().next().getDisplayName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.kill.success.multiple", targets.size()), true);
        }
