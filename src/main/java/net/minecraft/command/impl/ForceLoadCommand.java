...
public class ForceLoadCommand
{
    ...
    {
        return new TranslationTextComponent("cmd.forceload.toobig", p_212724_0_, p_212724_1_);
    });
    ...
    {
        return new TranslationTextComponent("cmd.forceload.query.failure", p_212717_0_, p_212717_1_);
    });
    private static final SimpleCommandExceptionType FAILED_ADD_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.forceload.added.failure"));
    private static final SimpleCommandExceptionType REMOVE_FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.forceload.removed.failure"));
    ...
    private static int doQuery(CommandSource p_212713_0_, ColumnPos p_212713_1_) throws CommandSyntaxException
    {
        ...
        if (flag)
        {
            p_212713_0_.sendFeedback(new TranslationTextComponent("cmd.forceload.query.success", chunkpos, registrykey.getLocation()), false);
            return 1;
        }
        else
        {
    ...
    private static int doList(CommandSource p_212721_0_)
    {
        ...
        if (i > 0)
        {
            ...
            if (i == 1)
            {
                p_212721_0_.sendFeedback(new TranslationTextComponent("cmd.forceload.list.single", registrykey.getLocation(), s), false);
            }
            else
            {
                p_212721_0_.sendFeedback(new TranslationTextComponent("cmd.forceload.list.multiple", i, registrykey.getLocation(), s), false);
            }
        }
        else
        {
            p_212721_0_.sendErrorMessage(new TranslationTextComponent("cmd.forceload.added.none", registrykey.getLocation()));
        }
    ...
    private static int removeAll(CommandSource p_212722_0_)
    {
        ...
        p_212722_0_.sendFeedback(new TranslationTextComponent("cmd.forceload.removed.all", registrykey.getLocation()), true);
        return 0;
    }
        ...
        if (i >= -30000000 && j >= -30000000 && k < 30000000 && l < 30000000)
        {
            ...
            else
            {
                ...
                else
                {
                    if (j2 == 1)
                    {
                        p_212719_0_.sendFeedback(new TranslationTextComponent("cmd.forceload." + (p_212719_3_ ? "added" : "removed") + ".single", chunkpos, registrykey.getLocation()), true);
                    }
                    else
                    {
                        ChunkPos chunkpos1 = new ChunkPos(i1, j1);
                        ChunkPos chunkpos2 = new ChunkPos(k1, l1);
                        p_212719_0_.sendFeedback(new TranslationTextComponent("cmd.forceload." + (p_212719_3_ ? "added" : "removed") + ".multiple", j2, registrykey.getLocation(), chunkpos1, chunkpos2), true);
                    }
