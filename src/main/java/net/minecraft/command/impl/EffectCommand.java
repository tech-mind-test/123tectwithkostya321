...
{
    private static final SimpleCommandExceptionType GIVE_FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.effect.give.failed"));
    private static final SimpleCommandExceptionType CLEAR_EVERYTHING_FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.effect.clear.everything.failed"));
    private static final SimpleCommandExceptionType CLEAR_SPECIFIC_FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.effect.clear.specific.failed"));
    ...
    private static int addEffect(CommandSource source, Collection <? extends Entity > targets, Effect effect, @Nullable Integer seconds, int amplifier, boolean showParticles) throws CommandSyntaxException
    {
        ...
        else
        {
            if (targets.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.effect.give.success.single", effect.getDisplayName(), targets.iterator().next().getDisplayName(), j / 20), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.effect.give.success.multiple", effect.getDisplayName(), targets.size(), j / 20), true);
            }
    ...
    private static int clearAllEffects(CommandSource source, Collection <? extends Entity > targets) throws CommandSyntaxException
    {
        ...
        else
        {
            if (targets.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.effect.clear.everything.success.single", targets.iterator().next().getDisplayName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.effect.clear.everything.success.multiple", targets.size()), true);
            }
    ...
    private static int clearEffect(CommandSource source, Collection <? extends Entity > targets, Effect effect) throws CommandSyntaxException
    {
        ...
        else
        {
            if (targets.size() == 1)
            {
                source.sendFeedback(new TranslationTextComponent("cmd.effect.clear.specific.success.single", effect.getDisplayName(), targets.iterator().next().getDisplayName()), true);
            }
            else
            {
                source.sendFeedback(new TranslationTextComponent("cmd.effect.clear.specific.success.multiple", effect.getDisplayName(), targets.size()), true);
            }
