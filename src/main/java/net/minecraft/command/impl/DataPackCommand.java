...
public class DataPackCommand
{
    ...
    private static int enablePack(CommandSource source, ResourcePackInfo pack, DataPackCommand.IHandler priorityCallback) throws CommandSyntaxException
    {
        ...
        source.sendFeedback(new TranslationTextComponent("cmd.datapack.modify.enable", pack.getChatLink(true)), true);
    ...
    private static int disablePack(CommandSource source, ResourcePackInfo pack)
    {
        ...
        List<ResourcePackInfo> list = Lists.newArrayList(resourcepacklist.getEnabledPacks());
        list.remove(pack);
        source.sendFeedback(new TranslationTextComponent("cmd.datapack.modify.disable", pack.getChatLink(true)), true);
        ReloadCommand.func_241062_a_(list.stream().map(ResourcePackInfo::getName).collect(Collectors.toList()), source);
        return list.size();
    }
    ...
    private static int listAvailablePacks(CommandSource source)
    {
        ...
        if (list.isEmpty())
        {
            source.sendFeedback(new TranslationTextComponent("cmd.datapack.list.available.none"), false);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.datapack.list.available.success", list.size(), TextComponentUtils.func_240649_b_(list, (p_198293_0_) ->
            {
    ...
    private static int listEnabledPacks(CommandSource source)
    {
        ...
        {
            source.sendFeedback(new TranslationTextComponent("cmd.datapack.list.enabled.none"), false);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.datapack.list.enabled.success", collection.size(), TextComponentUtils.func_240649_b_(collection, (p_198306_0_) ->
            {
