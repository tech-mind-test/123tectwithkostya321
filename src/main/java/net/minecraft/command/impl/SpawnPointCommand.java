...
public class SpawnPointCommand
{
    ...
    private static int setSpawnPoint(CommandSource source, Collection<ServerPlayerEntity> targets, BlockPos pos, float p_198696_3_)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.spawnpoint.success.single", pos.getX(), pos.getY(), pos.getZ(), p_198696_3_, s, targets.iterator().next().getDisplayName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.spawnpoint.success.multiple", pos.getX(), pos.getY(), pos.getZ(), p_198696_3_, s, targets.size()), true);
        }
