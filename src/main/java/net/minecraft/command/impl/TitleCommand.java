...
public class TitleCommand
{
    ...
    private static int clear(CommandSource source, Collection<ServerPlayerEntity> targets)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.title.cleared.single", targets.iterator().next().getDisplayName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.title.cleared.multiple", targets.size()), true);
        }
    ...
    private static int reset(CommandSource source, Collection<ServerPlayerEntity> targets)
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.title.reset.single", targets.iterator().next().getDisplayName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.title.reset.multiple", targets.size()), true);
        }
    ...
    private static int show(CommandSource source, Collection<ServerPlayerEntity> targets, ITextComponent message, STitlePacket.Type type) throws CommandSyntaxException
    {
        ...
        if (targets.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.title.show." + type.name().toLowerCase(Locale.ROOT) + ".single", targets.iterator().next().getDisplayName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.title.show." + type.name().toLowerCase(Locale.ROOT) + ".multiple", targets.size()), true);
        }
    ...
    private static int setTimes(CommandSource source, Collection<ServerPlayerEntity> target, int fadeIn, int stay, int fadeOut)
    {
        ...
        if (target.size() == 1)
        {
            source.sendFeedback(new TranslationTextComponent("cmd.title.times.single", target.iterator().next().getDisplayName()), true);
        }
        else
        {
            source.sendFeedback(new TranslationTextComponent("cmd.title.times.multiple", target.size()), true);
        }
