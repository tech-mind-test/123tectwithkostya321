...
public class EnchantCommand
{
    ...
    {
        return new TranslationTextComponent("cmd.enchant.failed.entity", p_208839_0_);
    });
    private static final DynamicCommandExceptionType ITEMLESS_EXCEPTION = new DynamicCommandExceptionType((p_208835_0_) ->
    {
        return new TranslationTextComponent("cmd.enchant.failed.itemless", p_208835_0_);
    });
    ...
    {
        return new TranslationTextComponent("cmd.enchant.failed.incompatible", p_208837_0_);
    });
    private static final Dynamic2CommandExceptionType INVALID_LEVEL = new Dynamic2CommandExceptionType((p_208840_0_, p_208840_1_) ->
    {
        return new TranslationTextComponent("cmd.enchant.failed.level", p_208840_0_, p_208840_1_);
    });
    private static final SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType(new TranslationTextComponent("cmd.enchant.failed"));
    ...
    private static int enchant(CommandSource source, Collection <? extends Entity > targets, Enchantment enchantmentIn, int level) throws CommandSyntaxException
    {
        ...
        else
        {
            ...
            else
            {
                if (targets.size() == 1)
                {
                    source.sendFeedback(new TranslationTextComponent("cmd.enchant.success.single", enchantmentIn.getDisplayName(level), targets.iterator().next().getDisplayName()), true);
                }
                else
                {
                    source.sendFeedback(new TranslationTextComponent("cmd.enchant.success.multiple", enchantmentIn.getDisplayName(level), targets.size()), true);
                }
