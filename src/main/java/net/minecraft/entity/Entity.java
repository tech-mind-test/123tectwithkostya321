...
public abstract class Entity implements INameable, ICommandSource {
    ...
    public void moveRelative(float p_213309_1_, Vector3d relative) {
        ...
        if (mc.player != null && this.getEntityId() == mc.player.getEntityId()) {
            EventManager.call(new EventOnMovePost(p_213309_1_, relative));
        }
    }
    ...
    /**
     * Checks if players can use this entity to access operator (permission level 2) cmd either directly or
     ...
     * unauthorized players to use restricted cmd
     */
