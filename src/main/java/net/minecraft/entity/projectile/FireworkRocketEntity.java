            ...
            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    if (this.boostedEntity instanceof ClientPlayerEntity) {
                        EventFireworkRocket event = new EventFireworkRocket(1.5F, 1.5F);
                        EventManager.call(event);

                        Vector3d look = this.boostedEntity.getLookVec();
                        Vector3d motion = this.boostedEntity.getMotion();
                        this.boostedEntity.setMotion(motion.add(
                                look.x * 0.1D + (look.x * event.getSpeed() - motion.x) * 0.5D,
                                look.y * 0.1D + (look.y * event.getYSpeed() - motion.y) * 0.5D,
                                look.z * 0.1D + (look.z * event.getSpeed() - motion.z) * 0.5D
                        ));
                    }
                }
