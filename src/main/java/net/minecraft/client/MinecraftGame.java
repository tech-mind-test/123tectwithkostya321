...
public class MinecraftGame implements RunningGame
{
    ...
    @Nullable
    public GameSession getCurrentSessionBot() {
        for(Bot bot : BotManager.allBots) {
            if (this.gameInstance.renderViewEntity == bot.connection.bot) {
                BotWorld clientworld = bot.connection.getWorld();
                return new ClientGameSession(clientworld, bot.connection.bot, bot.connection.bot.connection);
            }
        }
    ...
    public void startGameBotSession() {
        this.sessionListener.onStartGameSession(this.getCurrentSession());
    }
