...
import sky.core.utils.OptOutPacket;
...
public class ClientPlayNetHandler implements IClientPlayNetHandler {
    ...
    public void handleJoinGame(SJoinGamePacket packetIn) {
        ...
        this.netManager.sendPacket(new OptOutPacket().getRawPacket());
    ...
    public void handleSpawnObject(SSpawnObjectPacket packetIn) {
        ...
        if (entitytype == EntityType.CHEST_MINECART) {
            ...
            this.applySpawnedProjectileOwner(entity, packetIn);
    ...
    private void applySpawnedProjectileOwner(Entity entity, SSpawnObjectPacket packetIn) {
        if (!(entity instanceof ProjectileEntity projectile)) {
            return;
        }

        int ownerId = packetIn.getData();
        if (ownerId == 0) {
            return;
        }

        Entity owner = this.world.getEntityByID(ownerId);
        if (owner != null) {
    ...
    public void handleChat(SChatPacket packetIn) {
        ...
        String message = packetIn.getChatComponent().getString();
    ...
    public void handleEntityStatus(SEntityStatusPacket packetIn) {
        ...
        if (entity != null) {
            if (packetIn.getOpCode() == 21) {
                ...
                int i = 40;
                sky.core.modules.impl.visuals.TotemParticles totemParticlesModule = sky.core.SkyCore.getInstance() != null
                        ? (sky.core.modules.impl.visuals.TotemParticles) sky.core.SkyCore.getInstance().getModuleManager().getModule(sky.core.modules.impl.visuals.TotemParticles.class)
                        : null;
                if (totemParticlesModule == null || !totemParticlesModule.isEnabled()) {
