...
import sky.core.modules.impl.visuals.ArmorDurabilityView;
...
public class BipedArmorLayer<T extends LivingEntity, M extends BipedModel<T>, A extends BipedModel<T>> extends LayerRenderer<T, M> {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn instanceof AbstractClientPlayerEntity player) {
            Minecraft mc = Minecraft.getInstance();
            boolean isLocalPlayer = player == mc.player;
            boolean isFriend = SkyCore.getInstance().getFriendManager().isFriend(player.getName().getString());
            CustomModels customModels = (CustomModels) SkyCore.getInstance().getModuleManager().getModule(CustomModels.class);
            if (customModels != null && customModels.isEnabled() && (isLocalPlayer || (customModels.friends.get() && isFriend))) {
                return;
            }
        }
    ...
    private void func_241739_a_(MatrixStack p_241739_1_, IRenderTypeBuffer p_241739_2_, T p_241739_3_, EquipmentSlotType p_241739_4_, int p_241739_5_, A p_241739_6_) {
        ...
        if (itemstack.getItem() instanceof ArmorItem armoritem) {
            ...
            if (armoritem.getEquipmentSlot() == p_241739_4_) {
                ...
                float[] rgb;
                if (SkyCore.getInstance().getModuleManager().getModule(ArmorDurabilityView.class).isEnabled()) {
