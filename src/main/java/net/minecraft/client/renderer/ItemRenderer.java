...
import sky.core.modules.impl.visuals.ItemReplacer;
...
public class ItemRenderer implements IResourceManagerReloadListener
{
    ...
    public void renderItem(ItemStack itemStackIn, ItemCameraTransforms.TransformType transformTypeIn, boolean leftHand, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn, IBakedModel modelIn)
    {
        if (!itemStackIn.isEmpty())
        {
            ...
            boolean physicsEnabled = SkyCore.getInstance().getModuleManager().getModule(ItemPhysics.class).isEnabled() && ItemPhysics.size.get();
            if (isGroundItem && physicsEnabled) {
                matrixStackIn.scale(0.5f, 0.5f, 0.5f);
            }

            if (ItemReplacer.renderReplacement(itemStackIn, transformTypeIn, matrixStackIn, bufferIn, combinedLightIn, combinedOverlayIn))
            {
                matrixStackIn.pop();
                return;
            }
