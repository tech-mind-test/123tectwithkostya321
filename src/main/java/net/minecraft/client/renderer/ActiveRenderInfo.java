...
public class ActiveRenderInfo {
    ...
    public void update(IBlockReader worldIn, Entity renderViewEntity, boolean thirdPersonIn, boolean thirdPersonReverseIn, float partialTicks) {
        ...
        float renderYaw = renderViewEntity.getYaw(partialTicks);
        float renderPitch = renderViewEntity.getPitch(partialTicks);

        double posX = MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosX, renderViewEntity.getPosX());
        double posY = MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosY, renderViewEntity.getPosY()) + (double) MathHelper.lerp(partialTicks, this.previousHeight, this.height);
        double posZ = MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosZ, renderViewEntity.getPosZ());
        double cameraDistance = 4.0D;

        EventRotation event = new EventRotation(
                new Vector3d(posX, posY, posZ),
                new Vector2f(renderYaw, renderPitch)
        );
        ...
        if (thirdPersonIn) {
            ...
            this.movePosition(-this.calcCameraDistance(cameraDistance), 0.0D, 0.0D);
