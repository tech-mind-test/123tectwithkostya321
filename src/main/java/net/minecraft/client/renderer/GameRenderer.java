...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    public void getBotMouseOver(float partialTicks) {
        for(Bot bot1 : BotManager.allBots) {
            if (bot1 != null && bot1.connection.botController != null && bot1.connection.getWorld() != null) {
                double d0 = (double)bot1.connection.botController.getBlockReachDistance();
                bot1.connection.bot.objectMouseOver = bot1.connection.bot.pick(d0, partialTicks, false);
                Vector3d vector3d = bot1.connection.bot.getEyePosition(partialTicks);
                boolean flag = false;
                int i = 3;
                double d1 = d0;
                if (bot1.connection.botController.extendedReach()) {
                    d1 = (double)6.0F;
                    d0 = d1;
                } else {
                    if (d0 > (double)3.0F) {
                ...
                if (bot1.connection.bot.objectMouseOver != null) {
                    d1 = bot1.connection.bot.objectMouseOver.getHitVec().squareDistanceTo(vector3d);
                }

                Vector3d vector3d1 = bot1.connection.bot.getLook(1.0F);
                Vector3d vector3d2 = vector3d.add(vector3d1.x * d0, vector3d1.y * d0, vector3d1.z * d0);
                float f = 1.0F;
                AxisAlignedBB axisalignedbb = bot1.connection.bot.getBoundingBox().expand(vector3d1.scale(d0)).grow((double)1.0F, (double)1.0F, (double)1.0F);
                EntityRayTraceResult entityraytraceresult = ProjectileHelper.rayTraceEntities(bot1.connection.bot, vector3d, vector3d2, axisalignedbb, (p_lambda$getMouseOver$0_0_) -> !p_lambda$getMouseOver$0_0_.isSpectator() && p_lambda$getMouseOver$0_0_.canBeCollidedWith(), d1);
                if (entityraytraceresult != null) {
                    Entity entity1 = entityraytraceresult.getEntity();
                    Vector3d vector3d3 = entityraytraceresult.getHitVec();
                    double d2 = vector3d.squareDistanceTo(vector3d3);
                    if (flag && d2 > (double)9.0F) {
                        bot1.connection.bot.objectMouseOver = BlockRayTraceResult.createMiss(vector3d3, Direction.getFacingFromVector(vector3d1.x, vector3d1.y, vector3d1.z), new BlockPos(vector3d3));
                    } else if (d2 < d1 || bot1.connection.bot.objectMouseOver == null) {
                        bot1.connection.bot.objectMouseOver = entityraytraceresult;
                    }
                }
            }
        }
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            Animation animationModule2 = (Animation) SkyCore.getInstance().getModuleManager().getModule(Animation.class);
            ...
            boolean inWaterEyes = this.mc.player != null && this.mc.player.isInWater() && this.mc.player.areEyesInFluid(net.minecraft.tags.FluidTags.WATER);
            if (!fluidstate.isEmpty() && !inWaterEyes) {
                d0 = d0 * 60.0D / 70.0D;
            }

            if (!inWaterEyes) {
                d0 = Math.min(d0, 150.0D);
            }
        ...
        if (!this.debugView) {
            ...
            if (p_renderHand_4_) {
                ...
                if (eventThirdPersonRender.isThirdperson() && !flag && !this.mc.gameSettings.hideGUI && this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR) {
                    ...
                    if (bot1 != null) {
                        if (Config.isShaders()) {
                            ShadersRender.renderItemFP(this.itemRenderer, p_renderHand_3_, p_renderHand_1_, this.renderTypeBuffers.getBufferSource(), this.mc.getBotPlayer(), this.mc.getRenderManager().getPackedLight(this.mc.getBotPlayer(), p_renderHand_3_), p_renderHand_6_);
                        } else {
                            this.itemRenderer.renderItemInFirstPerson(p_renderHand_3_, p_renderHand_1_, this.renderTypeBuffers.getBufferSource(), this.mc.getBotPlayer(), this.mc.getRenderManager().getPackedLight(this.mc.getBotPlayer(), p_renderHand_3_));
                        }
    ...
    public void resetProjectionMatrix(Matrix4f matrixIn) {
        if (!RenderSystem.isOnRenderThread()) {
            Matrix4f matrixCopy = matrixIn.copy();
            RenderSystem.recordRenderCall(() -> this.resetProjectionMatrix(matrixCopy));
            return;
        }
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime, boolean renderWorldIn) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            label160:
            {
                i = (int) (this.mc.mouseHelper.getMouseX() * (double) this.mc.getMainWindow().getScaledWidth() / (double) this.mc.getMainWindow().getWidth());
                j = (int) (this.mc.mouseHelper.getMouseY() * (double) this.mc.getMainWindow().getScaledHeight() / (double) this.mc.getMainWindow().getHeight());
                if (renderWorldIn) {
                    label158:
                    {
                        if (bot1 != null) {
                            if (bot1.connection.getWorld() == null) {
                                break label158;
                            }
                        ...
                        if (!Config.isReloadingResources()) {
            ...
            RenderSystem.matrixMode(5889);
            RenderSystem.loadIdentity();
            RenderSystem.ortho((double) 0.0F, (double) mainwindow.getFramebufferWidth() / mainwindow.getGuiScaleFactor(), (double) mainwindow.getFramebufferHeight() / mainwindow.getGuiScaleFactor(), (double) 0.0F, (double) 1000.0F, (double) 3000.0F);
            RenderSystem.matrixMode(5888);
            RenderSystem.loadIdentity();
            RenderSystem.translatef(0.0F, 0.0F, -2000.0F);
            RenderHelper.setupGui3DDiffuseLighting();
            ...
            if (renderWorldIn) {
                label203:
                {
                    ...
                    label186:
                    {
                        ...
                        float f = MathHelper.lerp(partialTicks, bot1 != null ? this.mc.getBotPlayer().prevTimeInPortal : this.mc.player.prevTimeInPortal, bot1 != null ? this.mc.getBotPlayer().timeInPortal : this.mc.player.timeInPortal);
                        if (f > 0.0F) {
                            label140:
                            {
                                if (bot1 != null) {
                                ...
                                } else if (!this.mc.player.isPotionActive(Effects.NAUSEA)) {
                                    break label140;
                                }

                                if (this.mc.gameSettings.screenEffectScale < 1.0F) {
            ...
            if (this.mc.loadingGui != null) {
                ...
                try {
                    if (Reflector.ForgeHooksClient_drawScreen.exists()) {
                        Reflector.callVoid(Reflector.ForgeHooksClient_drawScreen, new Object[]{this.mc.currentScreen, matrixstack, i, j, this.mc.getTickLength()});
                    } else {
        ...
        if (!this.drawBlockOutline) {
            ...
            if (flag && !((PlayerEntity) entity).abilities.allowEdit) {
                ...
                if (raytraceresult != null && raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
                    ...
                    label123:
                    {
                        blockpos = ((BlockRayTraceResult) raytraceresult).getPos();
                        blockstate = bot1 != null ? bot1.connection.getWorld().getBlockState(blockpos) : this.mc.world.getBlockState(blockpos);
                        if (bot1 != null) {
                            if (bot1.connection.botController.getCurrentGameType() == GameType.SPECTATOR) {
                                break label123;
                            }
                        ...
                        CachedBlockInfo cachedblockinfo = new CachedBlockInfo((IWorldReader) (bot1 != null ? bot1.connection.getWorld() : this.mc.world), blockpos, false);
                        flag = !itemstack.isEmpty() && (itemstack.canDestroy(bot1 != null ? bot1.connection.getWorld().getTags() : this.mc.world.getTags(), cachedblockinfo) || itemstack.canPlaceOn(bot1 != null ? bot1.connection.getWorld().getTags() : this.mc.world.getTags(), cachedblockinfo));
                        return flag;
                    }

                    flag = blockstate.getContainer((World) (bot1 != null ? bot1.connection.getWorld() : this.mc.world), blockpos) != null;
                }
            }
    ...
    public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn) {
        ...
        this.getBotMouseOver(partialTicks);
        ...
        float f = MathHelper.lerp(partialTicks, bot1 != null ? this.mc.getBotPlayer().prevTimeInPortal : this.mc.player.prevTimeInPortal, bot1 != null ? this.mc.getBotPlayer().timeInPortal : this.mc.player.timeInPortal) * this.mc.gameSettings.screenEffectScale * this.mc.gameSettings.screenEffectScale;

        if (f > 0.0F) {
            int count;
            if (this.mc.player.isPotionActive(Effects.NAUSEA) || (bot1 != null && this.mc.getBotPlayer().isPotionActive(Effects.NAUSEA))) {
                count = 7;
            } else {
                count = 20;
            }
        ...
        activerenderinfo.update(world, (Entity) (bot1 != null ? this.mc.getBotPlayer() : this.mc.player),
                !this.mc.gameSettings.getPointOfView().firstPerson(),
                this.mc.gameSettings.getPointOfView() == PointOfView.THIRD_PERSON_FRONT,
                partialTicks);
        ...
        EventWorld3D eventWorld3D = new EventWorld3D(partialTicks, matrixStackIn, matrix4f);
        EventManager.call(eventWorld3D);
    ...
    private void frameFinish() {
        ...
        } else if (this.mc.world == null || !Config.isShowGlErrors() || !TimedEvent.isActive("CheckGlErrorFrameFinish", 10000L)) {
            return;
        }
        ...
        if (i != 0 && GlErrors.isEnabled(i)) {
            ...
            StringTextComponent stringtextcomponent = new StringTextComponent(I18n.format("of.message.openglError", new Object[]{i, s}));
            this.mc.ingameGUI.getChatGUI().printChatMessage(stringtextcomponent);
        }
