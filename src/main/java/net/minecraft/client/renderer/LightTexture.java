...
import sky.core.modules.impl.visuals.FullBright;
...
public class LightTexture implements AutoCloseable
{
    ...
    public void updateLightmap(float partialTicks)
    {
        ...
        for(Bot bot : BotManager.allBots) {
            if (this.client.renderViewEntity == bot.connection.bot) {
                bot1 = bot;
            }
        }
        if (this.needsUpdate)
        {
            ...
            World world = (World)(bot1 != null ? bot1.connection.getWorld() : this.client.world);
            if (world instanceof ClientWorld) {
                ClientWorld w = (ClientWorld) world;
                if (w != null) {
                    this.custom = false;
                    ...
                    if (Config.isCustomColors()) {
                        ...
                        if (CustomColors.updateLightmap(w, this.torchFlicker, this.nativeImage, flag, partialTicks)) {
                            this.dynamicTexture.updateDynamicTexture();
                            this.needsUpdate = false;
                    ...
                    float f9 = w.getSunBrightness(1.0F);
                    float f;

                    if (w.getTimeLightningFlash() > 0) {
                        f = 1.0F;
                    } else {
                        f = f9 * 0.95F + 0.05F;
                    }
                    ...
                    for (int i = 0; i < 16; ++i) {
                        for (int j = 0; j < 16; ++j) {
                            float f4 = this.getLightBrightness(w, i) * f;
                            float f5 = this.getLightBrightness(w, j) * f3;
                            float f6 = f5 * ((f5 * 0.6F + 0.4F) * 0.6F + 0.4F);
                            float f7 = f5 * (f5 * f5 * 0.6F + 0.4F);
                            vector3f1.set(f5, f6, f7);

                            if (w.func_239132_a_().func_241684_d_()) {
                            ...
                            float f11 = FullBright.getGammaForRendering((float) this.client.gameSettings.gamma);
            ...
            if (bot1 != null && world instanceof BotWorld) {
                BotWorld w = (BotWorld)world;
                if (w != null) {
                    this.custom = false;
                    if (Config.isCustomColors()) {
                        boolean flag = bot1.connection.bot.isPotionActive(Effects.NIGHT_VISION) || bot1.connection.bot.isPotionActive(Effects.CONDUIT_POWER);
                        if (CustomColors.updateLightmap(w, this.torchFlicker, this.nativeImage, flag, partialTicks)) {
                            this.dynamicTexture.updateDynamicTexture();
                            this.needsUpdate = false;
                            this.client.getProfiler().endSection();
                    ...
                    float f1 = bot1.connection.bot.getWaterBrightness();
                    float f2;
                    if (bot1.connection.bot.isPotionActive(Effects.NIGHT_VISION)) {
                        f2 = GameRenderer.getNightVisionBrightness(bot1.connection.bot, partialTicks);
                    } else if (f1 > 0.0F && bot1.connection.bot.isPotionActive(Effects.CONDUIT_POWER)) {
                        f2 = f1;
                    } else {
                        f2 = 0.0F;
                    }
