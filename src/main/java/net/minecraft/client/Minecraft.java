...
import other.bot.impl.BotTapeMouse;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        try {
            InputStream inputstream = Minecraft.class.getResourceAsStream("/assets/minecraft/SkyCore/icons/avatarka/avatar.png");
            InputStream inputstream1 = Minecraft.class.getResourceAsStream("/assets/minecraft/SkyCore/icons/avatarka/avatar.png");

            if (inputstream == null || inputstream1 == null) {
                throw new IOException("Custom icon not found: /assets/minecraft/SkyCore/icons/avatarka/avatar.png");
            }

            this.mainWindow.setWindowIcon(inputstream, inputstream1);
        } catch (IOException ioexception) {
            LOGGER.error("Couldn't set custom icon", ioexception);
        }
    ...
    public void displayGuiScreen(@Nullable Screen guiScreenIn) {
        ...
        for (Bot bot : BotManager.allBots) {
            if (this.renderViewEntity == bot.connection.bot) {
                bot1 = bot;
            }
        }
        ...
        if (guiScreenIn == null && this.world == null) {
            ...
            {
                if (bot1 != null) {
                    if (!bot1.connection.bot.getShouldBeDead()) {
                        break label73;
                    }
                } else if (this.player == null || !this.player.getShouldBeDead()) {
                    break label73;
                }
                ...
                {
                    if (bot1 != null) {
                        if (bot1.connection.bot.isShowDeathScreen()) {
                            break label50;
                        }
                    } else if (this.player.isShowDeathScreen()) {
                        break label50;
                    }
                    ...
                    if (bot1 == null) {
                        ...
                        bot1.connection.bot.respawnPlayer();
                    }
                    break label73;
                }
    ...
    public void shutdownMinecraftApplet() {
        try {
            ...
            LOGGER.info("остановка сердца бабушки ирибы oh no!");
    ...
    private void sendClickBlockToController(boolean leftClick) {
        ...
        if (this.leftClickCounter <= 0) {
            if (bot1 != null) {
                if (bot1.connection.bot.isHandActive()) {
                    return;
                }
            } else if (this.player.isHandActive()) {
                return;
            }
            if (leftClick && this.objectMouseOver != null && this.objectMouseOver.getType() == RayTraceResult.Type.BLOCK) {
                ...
                if (!world.getBlockState(blockpos).isAir()) {
                    ...
                    for (Bot bot : BotManager.allBots) {
                        if (this.renderViewEntity == bot.connection.bot) {
                            if (bot.connection.botController.onPlayerDamageBlock(blockpos, direction)) {
                                this.particles.addBlockHitEffects(blockpos, direction);
                                bot.connection.bot.swingArm(Hand.MAIN_HAND);
                            }
                    ...
                    EventManager.call(new EventBlockDamage(this.world.getBlockState(blockpos), blockpos, EventBlockDamage.State.START, direction));
                    if (!botsImprovements && this.playerController.onPlayerDamageBlock(blockpos, direction)) {
                        this.particles.addBlockHitEffects(blockpos, direction);
                        AttackOrder.sendConditionalSwing(this.objectMouseOver, Hand.MAIN_HAND);
                    }
                    EventManager.call(new EventBlockDamage(this.world.getBlockState(blockpos), blockpos, EventBlockDamage.State.STOP, direction));
                }
            } else {
                boolean botsImprovements = false;
                ...
                for (Bot bot : BotManager.allBots) {
                    if (this.renderViewEntity == bot.connection.bot) {
                        bot.connection.botController.resetBlockRemoving();
                        botsImprovements = true;
                    }
                }
    ...
    public void clickMouse() {
        Bot bot1 = null;
        Iterator var2 = BotManager.allBots.iterator();
        ...
        if (this.leftClickCounter <= 0) {
            if (this.objectMouseOver == null) {
                ...
                } else if (!bot1.connection.botController.isNotCreative()) {
                    return;
                }
                ...
                } else if (this.getBotPlayer().isRowingBoat()) {
                    return;
                }
                ...
                label99:
                {
                    switch (this.objectMouseOver.getType()) {
                        case ENTITY:
                            if (bot1 == null) {
                                ...
                                bot1.connection.botController.attackEntity(this.getBotPlayer(), ((EntityRayTraceResult) this.objectMouseOver).getEntity());
                            }
                            break label99;
                        case BLOCK:
                            label100:
                            {
                                ...
                                } else if (this.world.getBlockState(blockpos).isAir()) {
                                    break label100;
                                }

                                boolean botsImprovements = false;
                                Iterator var5 = BotManager.allBots.iterator();

                                while (var5.hasNext()) {
                                    Bot bot = (Bot) var5.next();
                                    if (this.renderViewEntity == bot.connection.bot) {
                                        bot.connection.botController.clickBlock(blockpos, blockraytraceresult.getFace());
                                        botsImprovements = true;
                                    }
                                }
                    ...
                    if (bot1 == null) {
                        ...
                        bot1.connection.bot.resetCooldown();
                    }
                }
                ...
                if (bot1 == null) {
                    ...
                    bot1.connection.bot.swingArm(Hand.MAIN_HAND);
                }
            }
        }
    ...
    public void rightClickMouse() {
        ...
        } else if (bot1.connection.botController.getIsHittingBlock()) {
            return;
        }
        ...
        if (bot1 != null) {
            if (bot1.connection.bot.isRowingBoat()) {
                return;
            }
        ...
        for (Hand hand : Hand.values()) {
            ...
            if (this.objectMouseOver != null) {
                switch (this.objectMouseOver.getType()) {
                    case ENTITY:
                        ...
                        if (bot1 == null) {
                            actionresulttype = this.playerController.interactWithEntity(this.player, entity, entityraytraceresult, hand);
                        } else {
                            actionresulttype = bot1.connection.botController.interactWithEntity(player, entity, entityraytraceresult, hand);
                        }
                    ...
                    case BLOCK:
                        float reach = (bot1 == null) ? playerController.getBlockReachDistance() : bot1.connection.botController.getBlockReachDistance();
                        RayTraceResult rayTraceResult = Rotation.rayTrace(reach, player.rotationYaw, player.rotationPitch, player, RayTraceContext.BlockMode.OPENWALLS);

                        if (rayTraceResult.getType() == RayTraceResult.Type.BLOCK && rayTraceResult instanceof BlockRayTraceResult traceResult && SkyCore.getInstance().getModuleManager().getOpenWalls().isEnabled() && player.getHeldItemMainhand().getItem() != Items.EXPERIENCE_BOTTLE) {
                            ActionResultType actionresulttype1;
                            if (bot1 == null) {
                                actionresulttype1 = playerController.func_217292_a((ClientPlayerEntity) player, world, hand, traceResult);
                            } else {
                                actionresulttype1 = bot1.connection.botController.func_217292_a(bot1.connection.bot, bot1.connection.getWorld(), hand, traceResult);
                            }
                        ...
                        if (bot1 == null) {
                            actionresulttype1 = this.playerController.func_217292_a(this.player, this.world, hand, blockraytraceresult);
                        } else {
                            actionresulttype1 = bot1.connection.botController.func_217292_a(bot1.connection.bot, bot1.connection.getWorld(), hand, blockraytraceresult);
                        }
                        ...
                        if (actionresulttype1.isSuccessOrConsume()) {
                            if (actionresulttype1.isSuccess()) {
                                ...
                                boolean isCreative = (bot1 != null) ? bot1.connection.botController.isInCreativeMode() : this.playerController.isInCreativeMode();

                                if (!itemstack.isEmpty() && (itemstack.getCount() != i || isCreative)) {
            ...
            EventRightClickItemCheck eventCheck = new EventRightClickItemCheck(itemstack, hand);
            EventManager.call(eventCheck);
            if (eventCheck.isCancelled()) {
                return;
            }
            ...
            if (!itemstack.isEmpty()) {
                ...
                if (bot1 != null) {
                    actionresulttype2 = bot1.connection.botController.processRightClick(player, bot1.connection.getWorld(), hand);
                } else {
    ...
    public void clickBotMouse() {
        for (BotTapeMouse bot : BotManager.tapeMouseBots) {
            bot.clickBotMouse();
        }
    ...
    public void rightClickBotMouse() {
        for (BotTapeMouse bot : BotManager.tapeMouseBots) {
            bot.rightClickBotMouse(this.gameRenderer);
        }
     ...
     */
    public void loadWorld(BotWorld worldClientIn) {
    ...
    private void updateWorldRenderer(@Nullable BotWorld worldIn) {
    ...
    public void runTick() {
        ...
        this.gameRenderer.getBotMouseOver(1.0F);
        ...
        if (!this.isGamePaused && this.world != null) {
            ...
            for (Bot bot : BotManager.allBots) {
                if (bot.connection.bot.getHealth() <= 0.0F) {
                    bot.connection.bot.respawnPlayer();
                    this.loadWorld(bot.connection.world);
                }

                bot.connection.botController.tick();
            }
        }
        ...
        if (this.world != null) {
            ...
            if (!this.isGamePaused) {
                ...
                for (Bot bot : BotManager.allBots) {
                    if (bot.connection.getWorld() != null) {
                        bot.connection.getWorld().tickEntities();
                    }
                }
            }
        ...
        if (this.world != null) {
            if (!this.isGamePaused) {
                ...
                try {
                    ...
                    for (Bot bot : BotManager.allBots) {
                        if (bot.connection.getWorld() != null) {
                            bot.connection.getWorld().tick(() -> true);
                        }
                    }
                } catch (Throwable throwable) {
            ...
            if (!this.isGamePaused && this.world != null) {
                ...
                for (Bot bot : BotManager.allBots) {
                    if (this.renderViewEntity == bot.connection.bot) {
                        bot.connection.getWorld().animateTick(MathHelper.floor(bot.connection.bot.getPosX()), MathHelper.floor(bot.connection.bot.getPosY()), MathHelper.floor(bot.connection.bot.getPosZ()));
                    }
                }
            }
            ...
            for (Bot bot : BotManager.allBots) {
                bot.connection.getBotNetwork().tick();
            }
        }
    ...
    private void processKeyBinds() {
        ...
        for (int i = 0; i < 9; ++i) {
            boolean isSaveKey = this.gameSettings.keyBindSaveToolbar.isKeyDown();
            boolean isLoadKey = this.gameSettings.keyBindLoadToolbar.isKeyDown();

            if (this.gameSettings.keyBindsHotbar[i].isPressed()) {
                boolean isSpectator = (bot1 != null) ? bot1.connection.bot.isSpectator() : this.player.isSpectator();

                if (isSpectator) {
                    this.ingameGUI.getSpectatorGui().onHotbarSelected(i);
                } else {
                    boolean isCreative = (bot1 != null) ? bot1.connection.bot.isCreative() : this.player.isCreative();

                    if (isCreative && this.currentScreen == null && (isSaveKey || isLoadKey)) {
                        CreativeScreen.handleHotbarSnapshots(this, i, isLoadKey, isSaveKey);
                    } else {
                        ...
                        if (!eventCancelHotbar.isCancelled()) {
                            if (bot1 == null) {
                                ...
                                bot1.connection.bot.inventory.currentItem = i;
                            }
                        }
                    }
                }
        ...
        while (this.gameSettings.field_244602_au.isPressed()) {
            if (!this.func_244600_aM()) {
                if (bot1 == null) {
                    ...
                    bot1.connection.bot.sendStatusMessage(field_244596_I, true);
                }
        ...
        while (true) {
            while (true) {
                if (!this.gameSettings.keyBindInventory.isPressed()) {
                    ...
                    while (this.gameSettings.keyBindSwapHands.isPressed()) {
                        if (bot1 != null) {
                            if (bot1.connection.bot.isSpectator()) {
                                continue;
                            }
                        ...
                        if (bot1 == null) {
                            ...
                            bot1.connection.bot.connection.sendPacket(new CPlayerDiggingPacket(CPlayerDiggingPacket.Action.SWAP_ITEM_WITH_OFFHAND, BlockPos.ZERO, Direction.DOWN));
                        }
                    }
                    ...
                    while (this.gameSettings.keyBindDrop.isPressed()) {
                        ...
                        if (!isSpectator) {
                            ...
                            if (!eventDropItem.isCancelled()) {
                                if (bot1 != null) {
                                    if (bot1.connection.bot.drop(Screen.hasControlDown())) {
                                        bot1.connection.bot.swingArm(Hand.MAIN_HAND);
                                    }
                                } else {
                                    if (this.player.drop(Screen.hasControlDown())) {
                    ...
                    if (flag2) {
                        ...
                        if (this.loadingGui == null && this.gameSettings.keyBindCommand.isPressed() && this.currentScreen == null) {
                            this.openChatScreen("/");
                        }
                    }
                    ...
                    label337:
                    {
                        ...
                        if (!this.gameSettings.keyBindUseItem.isKeyDown()) {
                            ...
                            for (Bot bot : BotManager.allBots) {
                                if (this.renderViewEntity == bot.connection.bot) {
                                    bot.connection.botController.onStoppedUsingItem(bot.connection.bot);
                                    botsImprovements = true;
                                }
                            }
                ...
                if (bot1 != null) {
                    if (bot1.connection.botController.isRidingHorse()) {
                        break;
                    }
                } else if (this.playerController.isRidingHorse()) {
                    break;
                }
                ...
                for (Bot bot : BotManager.allBots) {
                    if (this.renderViewEntity == bot.connection.bot) {
                        this.displayGuiScreen(new InventoryScreen(bot.connection.bot));
                        botsImprovements = true;
                    }
                }
            ...
            if (bot1 == null) {
                ...
                bot1.connection.bot.sendHorseInventory();
            }
        }
    }
