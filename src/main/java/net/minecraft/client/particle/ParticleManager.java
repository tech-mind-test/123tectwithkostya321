...
import mods.maseffects.MasEffectParticleTypes;
import mods.maseffects.particles.ReviveParticle;
import mods.maseffects.particles.ReviveSparkParticle;
import mods.particular.ParticularParticleTypes;
import mods.particular.particles.WaterSplashEmitterParticle;
import mods.particular.particles.WaterSplashFoamParticle;
import mods.particular.particles.WaterSplashParticle;
import mods.particular.particles.WaterSplashRingParticle;
...
public class ParticleManager implements IFutureReloadListener
{
    ...
    private void registerFactories()
    {
        ...
        this.registerFactory(ParticularParticleTypes.WATER_SPLASH_EMITTER, WaterSplashEmitterParticle.Factory::new);
        this.registerFactory(ParticularParticleTypes.WATER_SPLASH, WaterSplashParticle.Factory::new);
        this.registerFactory(ParticularParticleTypes.WATER_SPLASH_FOAM, WaterSplashFoamParticle.Factory::new);
        this.registerFactory(ParticularParticleTypes.WATER_SPLASH_RING, WaterSplashRingParticle.Factory::new);
        this.registerFactory(MasEffectParticleTypes.REVIVE, ReviveParticle.Factory::new);
        this.registerFactory(MasEffectParticleTypes.REVIVE_SPARK, ReviveSparkParticle.Factory::new);
    }
    ...
    public void renderParticles(MatrixStack p_renderParticles_1_, IRenderTypeBuffer.Impl p_renderParticles_2_, LightTexture p_renderParticles_3_, ActiveRenderInfo p_renderParticles_4_, float p_renderParticles_5_, ClippingHelper p_renderParticles_6_)
    {
        ...
        for(IParticleRenderType iparticlerendertype : collection) {
            if (iparticlerendertype != IParticleRenderType.NO_RENDER) {
                runnable.run();
                Iterable<Particle> iterable = (Iterable)this.byType.get(iparticlerendertype);
                if (iterable != null) {
                    ...
                    for(Particle particle : iterable) {
                        if (p_renderParticles_6_ == null || !particle.shouldCull() || p_renderParticles_6_.isBoundingBoxInFrustum(particle.getBoundingBox())) {
                            try {
                                if (flag || !(particle instanceof UnderwaterParticle) || particle.motionX != (double)0.0F || particle.motionY != (double)0.0F || particle.motionZ != (double)0.0F) {
                                ...
                                crashreportcategory.addDetail("Particle", particle::toString);
                                Objects.requireNonNull(iparticlerendertype);
