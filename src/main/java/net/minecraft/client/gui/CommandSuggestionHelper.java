...
public class CommandSuggestionHelper {
    ...
    public void init() {
        ...
        if (reader.canRead(SkyCore.getInstance().getCommandsManager().getPrefix().length()) && reader.getString().startsWith(SkyCore.getInstance().getCommandsManager().getPrefix(), reader.getCursor())) {
            reader.setCursor(reader.getCursor() + 1);

            if (parseResults == null) {
                parseResults = SkyCore.getInstance().getCommandsManager().getDispatcher().parse(reader, SkyCore.getInstance().getCommandsManager().getSource());
            }

            final int cursor = inputField.getCursorPosition();

            if (cursor >= 1 && (suggestions == null || !isApplyingSuggestion)) {
                suggestionsFuture = SkyCore.getInstance().getCommandsManager().getDispatcher().getCompletionSuggestions(parseResults, cursor);
        ...
        if (wantCommands) {
            CommandDispatcher<ISuggestionProvider> dispatcher = bot1 != null ? bot1.connection.bot.connection.getCommandDispatcher() : this.mc.player.connection.getCommandDispatcher();

            if (this.parseResults == null) {
                ISuggestionProvider suggestionProvider = bot1 != null ? bot1.connection.bot.connection.getSuggestionProvider() : this.mc.player.connection.getSuggestionProvider();
                this.parseResults = dispatcher.parse(reader, suggestionProvider);
            }
            ...
            Collection<String> names = bot1 != null ? bot1.connection.bot.connection.getSuggestionProvider().getPlayerNames() : this.mc.player.connection.getSuggestionProvider().getPlayerNames();
