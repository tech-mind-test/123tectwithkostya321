...
public class RecipeBookGui extends AbstractGui implements IRenderable, IGuiEventListener, IRecipeUpdateListener, IRecipePlacer<Ingredient>
{
    ...
    public void init(int widthIn, int heightIn, Minecraft minecraftIn, boolean widthTooNarrowIn, RecipeBookContainer<?> containerIn)
    {
        ...
        for(Bot bot : BotManager.allBots) {
            if (minecraftIn.renderViewEntity == bot.connection.bot) {
                bot1 = bot;
            }
        }
        ...
        if (bot1 == null) {
            ...
            bot1.connection.bot.openContainer = containerIn;
        }
        this.recipeBook = bot1 != null ? bot1.connection.bot.getRecipeBook() : minecraftIn.player.getRecipeBook();
        this.timesInventoryChanged = bot1 != null ? bot1.connection.bot.inventory.getTimesChanged() : minecraftIn.player.inventory.getTimesChanged();
        if (this.isVisible())
        {
    ...
    public void initSearchBar(boolean widthTooNarrowIn)
    {
        ...
        if (bot1 == null) {
            ...
            bot1.connection.bot.inventory.accountStacks(this.stackedContents);
    ...
    public void tick() {
        if (this.isVisible()) {
            ...
            if (this.timesInventoryChanged != (bot1 != null ? bot1.connection.bot.inventory.getTimesChanged() : this.mc.player.inventory.getTimesChanged())) {
                this.updateStackedContents();
                this.timesInventoryChanged = bot1 != null ? bot1.connection.bot.inventory.getTimesChanged() : this.mc.player.inventory.getTimesChanged();
            }
            ...
            if (this.recipeBookPage.func_198955_a(mouseX, mouseY, button, (this.width - 147) / 2 - this.xOffset, (this.height - 166) / 2, 147, 166))
            {
                ...
                if (irecipe != null && recipelist != null)
                {
                    ...
                    if (bot1 == null) {
                        ...
                        bot1.connection.botController.sendPlaceRecipePacket(this.mc.player.openContainer.windowId, irecipe, Screen.hasShiftDown());
                    }
    ...
    protected void sendUpdateSettings() {
        ...
        if (bot1 != null) {
            if (bot1.connection.bot.connection == null) {
                return;
            }
        ...
        if (bot1 == null) {
            ...
            bot1.connection.bot.connection.sendPacket(new CUpdateRecipeBookStatusPacket(recipebookcategory, flag, flag1));
        }
