...
public class ScreenManager
{
    ...
    public static <T extends Container> void openScreenToBot(@Nullable ContainerType<T> type, Minecraft mc, int windowId, ITextComponent title, Bot bot) {
        if (type == null) {
            LOG.warn("Trying to open invalid screen with name: {}", title.getString());
        } else {
            IScreenFactory<T, ?> iscreenfactory = getFactory(type);
            if (iscreenfactory == null) {
                LOG.warn("Failed to create screen for menu type: {}", Registry.MENU.getKey(type));
            } else {
                iscreenfactory.createScreenToBot(title, type, mc, windowId, bot);
            }
        }
    ...
    interface IScreenFactory<T extends Container, U extends Screen & IHasContainer<T>>
    {
        ...
        default void createScreenToBot(ITextComponent title, ContainerType<T> type, Minecraft mc, int windowId, Bot bot) {
            U u = this.create(type.create(windowId, bot.connection.bot.inventory), bot.connection.bot.inventory, title);
            bot.connection.bot.openContainer = ((IHasContainer)u).getContainer();
            mc.displayGuiScreen(u);
        }
