...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void closeScreenAndDropStack() {
        ...
        super.closeScreen();
        if (!(this.mc.currentScreen instanceof DropDown)) {
    ...
    public void livingTick() {
        ...
        Sprint sprint = (Sprint) SkyCore.getInstance().getModuleManager().getModule(Sprint.class);
        if (sprint != null && sprint.isEnabled() && sprint.getMode().is("Пакетный") && !this.isSprinting()
                && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation()
                && flag4 && !this.isHandActive() && !this.isPotionActive(Effects.BLINDNESS)) {
            this.setSprinting(this.movementInput.forwardKeyDown);
        }
