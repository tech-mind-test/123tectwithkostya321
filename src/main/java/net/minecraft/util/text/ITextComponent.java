...
default <T> Optional<T> getComponent(ITextProperties.ITextAcceptor<T> acceptor)
    ...
    default <T> Optional<T> func_230534_b_(ITextProperties.IStyledTextAcceptor<T> acceptor, Style style) {
        String text = this.getUnformattedComponentText();

        if (SkyCore.getInstance() != null && SkyCore.getInstance().getModuleManager().getModule(NameProtect.class).isEnabled()) {
            text = NameProtect.replaceName(text);
        }
