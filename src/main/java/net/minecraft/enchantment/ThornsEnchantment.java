...
public class ThornsEnchantment extends Enchantment
{
    ...
    public void onUserHurt(LivingEntity user, Entity attacker, int level)
    {
        ...
        if (shouldHit(level, random))
        {
            if (attacker != null)
            {
                EventThornsDamage event = new EventThornsDamage(attacker, user, level, (float)getDamage(level, random));
                EventManager.call(event);
                if (!event.isCancelled())
                {
                    attacker.attackEntityFrom(DamageSource.causeThornsDamage(user), event.getDamage());
                }
            }
