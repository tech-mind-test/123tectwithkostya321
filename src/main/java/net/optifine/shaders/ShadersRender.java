...
public class ShadersRender
{
    ...
    public static void renderItemFP(FirstPersonRenderer itemRenderer, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer.Impl bufferIn, BotPlayer playerEntityIn, int combinedLightIn, boolean renderTranslucent) {
        Minecraft.getInstance().worldRenderer.renderedEntity = playerEntityIn;
        GlStateManager.depthMask(true);
        if (renderTranslucent) {
            GlStateManager.depthFunc(519);
    ...
    {
        activeRenderInfo.update(mc.world, (Entity)(mc.getRenderViewEntity() == null ? mc.player : mc.getRenderViewEntity()), !mc.gameSettings.getPointOfView().firstPerson(), mc.gameSettings.getPointOfView().firstPerson(), partialTicks);
    }
