...
public class Shaders
{
    ...
    private static void setProgramUniforms(ProgramStage programStage)
    {
        ...
        setProgramUniform1f(uniform_screenBrightness, sky.core.modules.impl.visuals.FullBright.getGammaForRendering((float) mc.gameSettings.gamma));
    ...
    public static void drawHorizon(MatrixStack matrixStackIn)
    {
        ...
        World var16 = currentWorld;
        if (var16 instanceof ClientWorld w) {
            d5 = -cameraPositionY + w.getWorldInfo().getVoidFogHeight() + (double)12.0F - (double)16.0F;
        }

        var16 = currentWorld;
        if (!(var16 instanceof BotWorld w)) {
        } else {
            d5 = -cameraPositionY + w.getWorldInfo().getVoidFogHeight() + (double)12.0F - (double)16.0F;
        }

        boolean flag = false;
        World var17 = currentWorld;
        if (var17 instanceof ClientWorld w) {
            flag = cameraPositionY < w.getWorldInfo().getVoidFogHeight();
        }

        var17 = currentWorld;
        if (var17 instanceof BotWorld w) {
            flag = cameraPositionY < w.getWorldInfo().getVoidFogHeight();
        }
