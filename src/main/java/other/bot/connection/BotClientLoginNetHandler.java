/* Decompiler 80ms, total 326ms, lines 151 */
...
public class BotClientLoginNetHandler implements IClientLoginNetHandler {
    ...
    public BotClientLoginNetHandler(BotNetwork networkManagerIn, Minecraft mcIn, @Nullable Screen previousScreen, Consumer<ITextComponent> statusMessageConsumerIn) {
    ...
    private ITextComponent joinServer(String serverHash) {
        try {
            this.getSessionService().joinServer(this.mc.getSession().getProfile("tewerryshka"), this.mc.getSession().getToken(), serverHash);
            return null;
