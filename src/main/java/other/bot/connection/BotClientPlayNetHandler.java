/* Decompiler 1466ms, total 2155ms, lines 2322 */
package other.bot.connection;
...
import net.minecraft.client.renderer.debug.BeeDebugRenderer.Bee;
import net.minecraft.client.renderer.debug.BeeDebugRenderer.Hive;
import net.minecraft.client.renderer.debug.PointOfInterestDebugRenderer.BrainInfo;
import net.minecraft.client.renderer.debug.PointOfInterestDebugRenderer.POIInfo;
...
import net.minecraft.network.play.server.SChangeGameStatePacket.State;
import net.minecraft.network.play.server.SCombatPacket.Event;
import net.minecraft.network.play.server.SEntityPropertiesPacket.Snapshot;
...
import other.bot.impl.BotClientSuggestionProvider;
...
public class BotClientPlayNetHandler implements IClientPlayNetHandler {
    ...
    public BotWorld.ClientWorldInfo field_239161_g_;
    ...
    public void handleJoinGame(SJoinGamePacket packetIn) {
        ...
        BotManager.farmiliBots.add(new Bot((BotNetwork)null, (BotWorld)null, (BotPlayer)null, (BotController)null, this));
        ...
        BotWorld.ClientWorldInfo clientworld$clientworldinfo = new BotWorld.ClientWorldInfo(Difficulty.NORMAL, packetIn.isHardcoreMode(), flag1);
        this.field_239161_g_ = clientworld$clientworldinfo;
        int var10007 = this.viewDistance;
        Minecraft var10008 = this.client;
        Objects.requireNonNull(var10008);
        this.world = new BotWorld(this, clientworld$clientworldinfo, registrykey, dimensiontype, var10007, var10008::getProfiler, this.client.worldRenderer, flag, packetIn.getHashedSeed());
        ...
        if (BotManager.allBots.stream().filter((bot1) -> {
            return bot1.connection.bot.getName().getString().equals(this.bot.getName().getString());
        }).count() == 0L) {
        ...
        this.client.getMinecraftGame().startGameBotSession();
    }
    ...
    public void handleSpawnPlayer(SSpawnPlayerPacket packetIn) {
        ...
        int i = packetIn.getEntityID();
        RemoteClientPlayerEntity remoteclientplayerentity = new RemoteClientPlayerEntity(this.world, this.getPlayerInfo(packetIn.getUniqueId()).getGameProfile());
    ...
    public void handlePlayerPosLook(SPlayerPositionLookPacket packetIn) {
        ...
        boolean flag = packetIn.getFlags().contains(Flags.X);
        boolean flag1 = packetIn.getFlags().contains(Flags.Y);
        boolean flag2 = packetIn.getFlags().contains(Flags.Z);
        double d0;
        double d1;
        if (flag) {
            d0 = vector3d.getX();
        ...
        if (packetIn.getFlags().contains(Flags.X_ROT)) {
            f1 += playerentity.rotationPitch;
        }

        if (packetIn.getFlags().contains(Flags.Y_ROT)) {
        ...
        this.netManager.sendPacket(new PositionRotationPacket(playerentity.getPosX(), playerentity.getPosY(), playerentity.getPosZ(), playerentity.rotationYaw, playerentity.rotationPitch, false));
    ...
    public void handleRespawn(SRespawnPacket packetIn) {
        ...
        if (registrykey != clientplayerentity.world.getDimensionKey()) {
            ...
            BotWorld.ClientWorldInfo clientworld$clientworldinfo = new BotWorld.ClientWorldInfo(this.field_239161_g_.getDifficulty(), this.field_239161_g_.isHardcore(), flag1);
            this.field_239161_g_ = clientworld$clientworldinfo;
            int var10007 = this.viewDistance;
            Minecraft var10008 = this.client;
            Objects.requireNonNull(var10008);
    ...
    public void handleChangeGameState(SChangeGameStatePacket packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        PlayerEntity playerentity = this.bot;
        State schangegamestatepacket$state = packetIn.func_241776_b_();
        ...
        if (schangegamestatepacket$state == SChangeGameStatePacket.field_241764_a_) {
            ...
            if (i == 0) {
                this.bot.connection.sendPacket(new CClientStatusPacket(net.minecraft.network.play.client.CClientStatusPacket.State.PERFORM_RESPAWN));
            } else if (i == 1) {
    ...
    public void handleUpdateRecipes(SUpdateRecipesPacket packetIn) {
        ...
        // Get the RecipeList objects from the client recipe book
        Collection<RecipeList> recipeLists = clientrecipebook.getRecipes();

        // Add each RecipeList to the search tree
        recipeLists.forEach(imutablesearchtree::func_217872_a);
    ...
    public void handleRecipeBook(SRecipeBookPacket packetIn) {
        ...
        net.minecraft.network.play.server.SRecipeBookPacket.State srecipebookpacket$state = packetIn.getState();
        Optional var10000;
        Iterator var4;
        ...
        switch(srecipebookpacket$state) {
            case REMOVE:
                ...
                while(true) {
                    ...
                    Optional<IRecipe<?>> recipeOptional = (Optional<IRecipe<?>>) this.recipeManager.getRecipe(resourcelocation2);
                    recipeOptional.ifPresent(recipe -> clientrecipebook.lock(recipe));
                }
            case INIT:
                var4 = packetIn.getRecipes().iterator();
                ...
                while(var4.hasNext()) {
                    resourcelocation2 = (ResourceLocation)var4.next();
                    Optional<IRecipe<?>> recipeOptional = (Optional<IRecipe<?>>) this.recipeManager.getRecipe(resourcelocation2);
                    recipeOptional.ifPresent(recipe -> clientrecipebook.unlock(recipe));
                }
                ...
                while(true) {
                    ...
                    resourcelocation2 = (ResourceLocation)var4.next();
                    Optional<?> optionalRecipe = this.recipeManager.getRecipe(resourcelocation2);

                    optionalRecipe.ifPresent(recipe -> {
                        if (recipe instanceof IRecipe) {
                            clientrecipebook.markNew((IRecipe<?>) recipe);
                        }
                    });
                }
            case ADD:
    ...
    public void handleCombatEvent(SCombatPacket packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        if (packetIn.eventType == Event.ENTITY_DIED) {
    ...
    public void handleTitle(STitlePacket packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        Type stitlepacket$type = packetIn.getType();
    ...
    public void handlePlayerListItem(SPlayerListItemPacket packetIn) {
        ...
        while(var2.hasNext()) {
            AddPlayerData splayerlistitempacket$addplayerdata = (AddPlayerData)var2.next();
            if (packetIn.getAction() == Action.REMOVE_PLAYER) {
                ...
                if (packetIn.getAction() == Action.ADD_PLAYER) {
    ...
    public void handleCustomSound(SPlaySoundPacket packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
        this.client.getSoundHandler().play(new SimpleSound(packetIn.getSoundName(), packetIn.getCategory(), packetIn.getVolume(), packetIn.getPitch(), false, 0, AttenuationType.LINEAR, packetIn.getX(), packetIn.getY(), packetIn.getZ(), false));
    }
        ...
        if (this.validateResourcePackUrl(s)) {
            if (s.startsWith("level://")) {
                try {
                    ...
                    if (file2.isFile()) {
                        this.sendResourcePackStatus(net.minecraft.network.play.client.CResourcePackStatusPacket.Action.ACCEPTED);
                ...
                this.sendResourcePackStatus(net.minecraft.network.play.client.CResourcePackStatusPacket.Action.FAILED_DOWNLOAD);
            } else {
                ServerData serverdata = this.client.getCurrentServerData();
                if (serverdata != null && serverdata.getResourceMode() == ServerResourceMode.ENABLED) {
                    this.sendResourcePackStatus(net.minecraft.network.play.client.CResourcePackStatusPacket.Action.ACCEPTED);
                    this.handlePackFuture(this.client.getPackFinder().downloadResourcePack(s, s1));
                } else if (serverdata != null && serverdata.getResourceMode() != ServerResourceMode.PROMPT) {
                    this.sendResourcePackStatus(net.minecraft.network.play.client.CResourcePackStatusPacket.Action.DECLINED);
                } else {
                    this.client.execute(() -> {
                        this.client.displayGuiScreen(new ConfirmScreen((p_217274_3_) -> {
                            ...
                            if (p_217274_3_) {
                                ...
                                if (serverdata1 != null) {
                                    serverdata1.setResourceMode(ServerResourceMode.DISABLED);
                                }
    ...
    private void handlePackFuture(CompletableFuture<?> futureIn) {
        futureIn.thenRun(() -> {
            this.sendResourcePackStatus(net.minecraft.network.play.client.CResourcePackStatusPacket.Action.SUCCESSFULLY_LOADED);
    ...
    private void sendResourcePackStatus(net.minecraft.network.play.client.CResourcePackStatusPacket.Action action) {
    ...
    public void handleOpenBookPacket(SOpenBookWindowPacket packetIn) {
        ...
        if (itemstack.getItem() == Items.WRITTEN_BOOK) {
            this.client.displayGuiScreen(new ReadBookScreen(new WrittenBookInfo(itemstack)));
        }
                    ...
                    if (SCustomPayloadPlayPacket.DEBUG_CAVES.equals(resourcelocation)) {
                        ...
                        if (SCustomPayloadPlayPacket.DEBUG_STRUCTURES.equals(resourcelocation)) {
                            ...
                            if (SCustomPayloadPlayPacket.DEBUG_POI_ADDED.equals(resourcelocation)) {
                                ...
                                j5 = packetbuffer.readInt();
                                POIInfo pointofinterestdebugrenderer$poiinfo = new POIInfo(blockpos8, s9, j5);
                                ...
                                for(i6 = 0; i6 < j5; ++i6) {
                                    ...
                                    list3.add(new net.minecraft.client.renderer.debug.EntityAIDebugRenderer.Entry(blockpos8, j6, s, flag));
                                }
                                ...
                                if (SCustomPayloadPlayPacket.DEBUG_BRAIN.equals(resourcelocation)) {
                                    ...
                                    BrainInfo pointofinterestdebugrenderer$braininfo = new BrainInfo(uuid1, k6, s1, s2, i1, f1, f2, iposition1, s3, path1, flag2);
                                    ...
                                    Bee beedebugrenderer$bee = new Bee(uuid1, k6, iposition1, path2, blockpos10, blockpos11, l6);
                                    ...
                                    if (SCustomPayloadPlayPacket.field_229728_n_.equals(resourcelocation)) {
                                        ...
                                        Hive beedebugrenderer$hive = new Hive(blockpos8, s9, j5, l5, flag3, this.world.getGameTime());
    ...
    public void handleTeams(STeamsPacket packetIn) {
        ...
        if (packetIn.getAction() == 0 || packetIn.getAction() == 2) {
            ...
            Visible team$visible = Visible.getByName(packetIn.getNameTagVisibility());
            if (team$visible != null) {
                scoreplayerteam.setNameTagVisibility(team$visible);
            }

            CollisionRule team$collisionrule = CollisionRule.getByName(packetIn.getCollisionRule());
    ...
    public void handleEntityProperties(SEntityPropertiesPacket packetIn) {
        ...
        if (entity != null) {
            if (!(entity instanceof LivingEntity)) {
                ...
                while(true) {
                    while(var4.hasNext()) {
                        Snapshot sentitypropertiespacket$snapshot = (Snapshot)var4.next();
