...
public interface CapeHolder {
    ...
    default void simulate(AbstractClientPlayerEntity abstractClientPlayer) {
        ...
        float heightMul = (float) Cape.config.heightMultiplier;
        final float straveMul = (float) Cape.config.straveMultiplier;
        ...
        if (abstractClientPlayer.canSwim()) {
            simulation.setGravity(Cape.config.gravity / 10.0f);
        } else {
            simulation.setGravity((float) Cape.config.gravity);
        }
