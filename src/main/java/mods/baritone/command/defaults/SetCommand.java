    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (resetting) {
            if (!args.hasAny()) {
                ...
                logDirect("ALL settings will be reset. Use the 'set modified' or 'modified' cmd to see what will be reset.");
        ...
        if (!doingSomething && !args.hasAny()) {
            ...
            if ((setting.getName().equals("chatControl") && !(Boolean) setting.value && !Baritone.settings().chatControlAnyway.value) ||
                ...
                logDirect("Warning: Chat cmd will no longer work. If you want to revert this change, use prefix control (if enabled) or click the old value listed above.", TextFormatting.RED);
            } else if (setting.getName().equals("prefixControl") && !(Boolean) setting.value) {
                logDirect("Warning: Prefixed cmd will no longer work. If you want to revert this change, use chat control (if enabled) or click the old value listed above.", TextFormatting.RED);
            }
        }
        SettingsUtil.save(Baritone.settings());
    }
